...
class http_event_collector:
    ...
    def _batchThread(self):
        ...
        if not self.batchEvents:
            return
        payload = "".join(self.batchEvents)
        headers = {'Authorization':'Splunk '+self.token, 'X-Splunk-Request-Channel':str(uuid.uuid1())}
        try:
            response = self.requests_retry_session().post(self.server_uri, data=payload, headers=headers, verify=self.SSL_verify)
            ...
            self.batchEvents = []  # Clear the current batch
            self.currentByteLength = 0  # Reset the byte counter

    def flushBatch(self):
        """Method called to force flushing of remaining batch events.
        Always call this method before exiting your code to ensure all events are sent.
        """
        if not self.batchEvents:  # Check if there are any events to send
            return
        self.log.debug("Manual Flush: Sending the batch directly.")
        self._batchThread()  # Direct call to send the batch
